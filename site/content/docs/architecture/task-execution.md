...
`script_divide` 额外执行结果完整性门禁：

- 非空章节不能以空 `shots` 写库并标记成功。
- 模型明确返回 `total_shots = 1`、但遗漏唯一镜头数组时，系统用任务中冻结的章节原文无损补成一个镜头。
- 其他空结果直接进入失败状态，避免任务中心显示成功而章节仍为“待提取分镜”。
- 非空 `shots` 的 `total_shots` 统一以实际数组长度为准；写库前再次验证数量一致。
...
模型管理中的 `text`、`image`、`video` 是独立能力。当前 OpenAI、火山引擎和阿里云百炼均可注册这三类模型：

- 文本模型统一通过 OpenAI 兼容协议构造聊天模型，使用供应商的通用 `base_url`。
- 图片和视频模型由任务注册表分派到各供应商适配器，分别优先使用 `image_base_url` 与 `video_base_url`。
- 阿里云图片适配器兼容同步的多模态生成接口和异步图片任务接口；视频适配器使用异步任务创建与轮询协议。
- “供应商连接测试”和文本模型“快速测试”会发送一个最小真实请求。图片、视频测试必须在对应生成工作台执行，避免管理页误触发高费用任务。
...
- 悬浮任务中心当前统一展示全局任务列表：
  - 默认隐藏，是否展开会按本地历史状态恢复；每次进入默认展示运行中任务
  - 首次进入时，悬浮按钮默认位于左下角
  ...
  - 统一执行取消操作
  - 支持回到对应来源页面
  - 后台运行态每 10 秒轮询一次，查询活跃任务及最近 60 秒结束的任务
  - 顶部统一表达“任务范围”：当前页面 / 未结束 / 已结束 / 全部；未结束含等待中、进行中、取消中，已结束含已完成、失败、已取消
  - 切换到“已结束”或“全部”时才按状态查询 MySQL 历史记录，不受近期时间窗限制
  - 历史记录使用服务端分页，面板每页最多展示 4 条任务
  - 桌面端采用 520 × 640 的宽体任务面板，小屏幕按视口自适应
  - 面板外框使用固定的自适应高度；历史查询通过内容区遮罩反馈加载状态，状态切换不会改变面板或悬浮操作按钮的位置
  - 筛选区与分页底栏固定，只有中间的任务列表区域滚动；任务较多时仍能直接看到总数和翻页按钮
  - 任务类型选项来自系统任务类型清单，不依赖当前是否恰好存在运行中任务
  - 翻页使用紧凑箭头控件，不引入大体积分页栏
  - 失败任务按需读取 `/tasks/{task_id}/result` 并展示持久化 `error`；列表本身不承载大结果或业务调试上下文
...
- 终态优先于 `cancel_requested` 展示；`cancelled + cancel_requested=true` 必须显示“已取消”，不能继续显示“取消中”
- `script_import_analyze` 取消或失败时同步投影 `ScriptImport`；详情读取还会对账最近任务终态，自动修复历史遗留的 `analyzing`
- 当前不承诺 provider / LLM SDK 层面的绝对强终止，只保证业务任务状态会立即收敛

异步生成任务持久化时会先单独 flush `GenerationTask` 父记录，再写入 Link、媒体快照和 Outbox。该顺序是 MySQL 外键约束的一部分，避免任务 API 返回 500、前端继而报告“缺少任务 ID”。
