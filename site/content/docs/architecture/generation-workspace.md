...
资产实体可通过 `AssetFileLink` 关联图片、视频、音频和文档。关联的核心语义是“可选参考”，不是生成前置条件：

- 未关联素材时，图片与视频继续使用原有文本、首帧、尾帧和关键帧流程。
- 资产图片生成会按“专用形象图 → 主参考附件 → 用户排序附件”推荐最多四张图片。
- 镜头关联资产没有专用形象图时，可回退到该资产启用的主图片附件。
- 前端生成预览允许逐张移除、全部禁用或恢复智能推荐；最终选中的 `file_id` 随任务参数持久化。
- 视频、音频和文档附件保留业务角色与说明，不会被错误地当作图片提交给图片模型。

### 镜头音轨与视频合成

`AudioAsset` 管理可复用配音、旁白、配乐、环境声、音效和转场音，`ShotAudioTrack` 保存镜头内的开始时间、音量、淡入淡出与循环设置。

视频生成完成后仅在镜头存在音轨时调用 FFmpeg 合成：

- 原始模型视频始终保留。
- 合成成功后，镜头和任务结果指向新的视频文件。
- FFmpeg 缺失、音频不可读或合成失败时，任务仍成功并回退原视频，同时在结果中写入 `composition_warning`。
- 没有音轨时完全跳过合成，不增加原有视频生成路径的开销。
