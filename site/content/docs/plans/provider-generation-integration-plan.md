...
截至 2026-09-03，P0–P3 已落地并进入自动化验证；P4 的真实付费端到端调用仍需逐供应商人工确认。
...
状态：已完成代码与 Mock 契约验证；真实付费调用归入 P4。
...
状态：已完成当前统一契约可表达范围内的模型能力、请求字段与前端动态控件；阿里参考音色已接入，供应商新增的自动配音、原生多镜头等其他专有参数不冒充已支持能力。
