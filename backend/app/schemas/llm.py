...
class VideoGenerationOptionsRead(BaseModel):
    ...
    supports_text_to_video: bool = Field(True, description="是否支持纯文本生成视频")
    supports_first_frame: bool = Field(True, description="是否支持首帧参考")
    ...
    max_seconds: int | None = Field(None, description="连续时长上限")
    ...
    supports_subject_audio_reference: bool = Field(False, description="是否支持主体参考音频/音色")
