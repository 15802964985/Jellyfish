...
5. 勾选需要导入的章节后点击“确认导入”。全部选中章节在同一数据库事务中创建；失败会整体回滚，重复点击不会重复建章。
6. “导入后自动启动 AI 分镜提取”默认关闭；开启会为已导入章节创建文本模型任务并产生费用。

原文件和解析草稿会保留，章节 `raw_text` 使用清理后的业务正文。长剧本建议先检查免费结构预览，再决定是否调用 AI；PDF 无文字层、乱码或未识别章节时不要直接提交。

当前阶段只打通了规范章节的选择和事务提交。演员、角色、场景、道具、服装、镜头、旁白、字幕、BGM/SFX 的整份剧本深度分析仍在 PL-010 后续阶段；接通后必须由用户主动点击，并明确提示正文将发送到当前默认文本模型供应商及可能产生费用。
