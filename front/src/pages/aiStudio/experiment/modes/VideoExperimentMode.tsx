...
function FrameControl({ slot, file, disabled, uploading, onUpload, onOpenLibrary, onRemove }: {
  ...
  return <div className="flex items-center gap-1">
    ...
    {file ? <div className="group relative h-9 w-9 overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm" title={`${label}：${file.name}`}>
      <PreviewImage src={buildFileDownloadUrl(file.id)} alt={`${label}：${file.name}`} className="h-full w-full object-cover" />
      <button type="button" aria-label={`移除${label}：${file.name}`} className="absolute right-0 top-0 flex h-4 w-4 items-center justify-center rounded-bl bg-slate-900/70 text-white" onClick={() => onRemove(slot)}><CloseOutlined /></button>
    </div> : null}
  </div>
}
...
export function VideoExperimentMode({ sessionId, ensureSession, clearSessionMessages, render }: VideoExperimentModeProps) {
  const [channel,setChannel]=useState<'api'|'web'>('api')
  ...
  const availableFrameSlots = (['first', 'last', 'key'] as FrameSlot[]).filter((slot) => {
    if (channel==='web'||!capability) return true
  ...
  const durationOptions = useMemo(() => {
    if(channel==='web')return Array.from({length:120},(_,index)=>index+1)
  ...
  const uploadSubjectMedia = async (subjectId: string, kind: SubjectMediaKind, file: UploadFile): Promise<boolean> => {
    ...
    if (channel==='api' && kind === 'image' && !capability?.supports_subject_image_reference) return false
    if (channel==='api' && kind === 'video' && !capability?.supports_subject_video_reference) return false
    if (channel==='api' && kind === 'audio' && !capability?.supports_subject_audio_reference) return false
    const subject = subjectReferences.find((item) => item.id === subjectId)
    if (!subject) return false
    if (channel==='api' && kind === 'image' && capability?.max_images_per_subject != null && subject.imageFileIds.length >= capability.max_images_per_subject) return message.warning(`每个主体最多支持 ${capability.max_images_per_subject} 张图片`), false
    if (channel==='api' && kind === 'video' && capability?.max_videos_per_subject != null && subject.videoFileIds.length >= capability.max_videos_per_subject) return message.warning(`每个主体最多支持 ${capability.max_videos_per_subject} 个视频`), false
    if (channel==='api' && kind === 'audio' && capability?.max_audios_per_subject != null && subject.audioFileIds.length >= capability.max_audios_per_subject) return message.warning(`每个主体最多支持 ${capability.max_audios_per_subject} 个参考音频`), false
    if (channel==='api' && capability?.max_media_per_subject != null && subject.imageFileIds.length + subject.videoFileIds.length + subject.audioFileIds.length >= capability.max_media_per_subject) return message.warning(`每个主体最多支持 ${capability.max_media_per_subject} 个参考素材`), false
    if (channel==='api' && kind === 'video' && capability?.max_total_subject_videos != null && subjectReferences.reduce((total, item) => total + item.videoFileIds.length, 0) >= capability.max_total_subject_videos) return message.warning(`当前模型最多支持 ${capability.max_total_subject_videos} 个主体视频`), false
    if(channel==='web'&&!canAppendSubjectMedia(subjectId,kind)){message.warning('每个交接主体最多 10 份素材');return false}
  ...
  const canAppendSubjectMedia = (subjectId: string, kind: SubjectMediaKind): boolean => {
    if(channel==='web'){const item=subjectReferences.find(s=>s.id===subjectId);return !!item&&item.imageFileIds.length+item.videoFileIds.length+item.audioFileIds.length<10}
    const subject = subjectReferences.find((item) => item.id === subjectId)
    if (!subject) return false
  ...
  const creativePreview = useCreativePrompt(sessionId ? 'lab' : 'global', sessionId || 'system', 'video', currentPrompt)
  ...
  const submit = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('jellyfish:task-accepted',{detail:{taskId:created.task_id,title:'实验室视频生成'}}))
  ...
  const subjectActions = channel==='web' || capability?.supports_subject_image_reference || capability?.supports_subject_video_reference ? <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 border-l border-slate-200 pl-2">
    ...
    <Button size="small" disabled={disabled || (channel==='api'&&hasFrameReferences)} onClick={() => setSubjectModalOpen(true)}>编辑主体参考</Button>
    {channel==='api'&&hasFrameReferences ? <span className="text-xs text-slate-500">关键帧已启用，不能添加主体参考</span> : null}
  </div> : null
  const subjectEditor = <Modal title="编辑主体参考" open={subjectModalOpen} onCancel={() => setSubjectModalOpen(false)} footer={<div className="flex justify-end gap-2"><Button onClick={() => setSubjectModalOpen(false)}>稍后完成</Button><Button type="primary" disabled={hasIncompleteSubject} onClick={() => setSubjectModalOpen(false)}>完成</Button></div>} width={820} destroyOnClose={false}>
    <div className="mb-3 flex flex-wrap items-center gap-2 text-sm text-slate-600">
      ...
      <Button size="small" disabled={disabled || (channel==='web'?subjectReferences.length>=5:(capability?.max_subjects != null && subjectReferences.length >= capability.max_subjects))} onClick={() => { const subject = { id: crypto.randomUUID(), name: '', imageFileIds: [], videoFileIds: [], audioFileIds: [] }; setSubjectReferences((current) => [...current, subject]) }}>添加主体</Button>
    </div>
  ...
  /** Preserve frame meaning and the originating session; API capabilities do not rename web models. */
  const prepareWeb = async () => {
    if (!creativePreview.prompt || creativePreview.error) throw new Error(creativePreview.error || '请等待提示词准备完成')
    const groups=subjectReferences.map(subject=>({name:subject.name,media:[...subject.imageFileIds.map(file_id=>({file_id,media_kind:'image' as const})),...subject.videoFileIds.map(file_id=>({file_id,media_kind:'video' as const})),...subject.audioFileIds.map(file_id=>({file_id,media_kind:'audio' as const}))]}))
    const prompt=creativePreview.prompt, duration=seconds, aspect=ratio
    const refs=[frameFileIds.first,frameFileIds.last,frameFileIds.key].filter((id):id is string=>!!id)
    const mode=groups.length?'subjects':frameFileIds.first&&frameFileIds.last&&!frameFileIds.key?'first_last_frames':frameFileIds.first&&refs.length===1?'first_frame':refs.length?'reference_images':'text'
    const session=await ensureSession('video')
    return {target_type:'lab_video' as const,entity_id:session.id,expected_version:1,prompt,reference_file_ids:refs,duration_seconds:duration,aspect_ratio:aspect,subject_groups:groups,reference_mode:mode as 'subjects'|'first_last_frames'|'first_frame'|'reference_images'|'text'}
  }
  const composer = <ExperimentComposer channel={channel} onChannelChange={setChannel} webAction={<WebHandoffButton prepare={prepareWeb} disabled={disabled}/>} submitting={disabled} submitDisabled={disabled} submitLabel="生成视频" onSubmit={() => void submit()} options={<>{channel==='api'&&<GenerationOptionsPanel category="video" modelId={modelId} ratio={ratio} quantity={seconds} onChange={setGenerationChoice} />}<ExperimentOptionBar models={models.map((item) => ({ id: item.id, name: item.name }))} templates={templates.map((item) => ({ id: item.id, name: item.name, version: item.version, preview: item.preview, category: '视频提示词' }))} modelId={modelId} templateId={templateId} modelsLoading={modelsLoading} templatesLoading={templatesLoading} disabled={disabled} modelLabel="API 视频模型" modelPlaceholder="选择已登记的视频模型" onModelChange={selectModel} onTemplateChange={selectTemplate} onModelOpenChange={(open) => { if (open) void loadModels() }} onTemplateOpenChange={(open) => { if (open) void loadTemplates() }} /></>} contextActions={<div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 border-l border-slate-200 pl-2">{availableFrameSlots.map((slot) => <FrameControl key={slot} slot={slot} file={selectedFrames[slot]} disabled={disabled || (channel==='api'&&hasSubjectReferences)} uploading={uploadingSlot === slot} onUpload={uploadFrame} onOpenLibrary={setLibraryTarget} onRemove={(target) => setFrameFileIds((current) => ({ ...current, [target]: undefined }))} />)}{channel==='api'&&hasSubjectReferences ? <span className="text-xs text-slate-500">主体参考已启用，不能添加关键帧</span> : null}<Select size="small" value={seconds} onChange={setSeconds} disabled={disabled} options={durationOptions.map((value) => ({ value, label: `${value} 秒` }))} aria-label="视频时长" /><Select size="small" value={ratio} onChange={setRatio} disabled={disabled} options={(channel==='web'?ratioOptions:capability?.allowed_ratios?.length ? capability.allowed_ratios : ratioOptions).map((value) => ({ value, label: value }))} aria-label="视频比例" />{subjectActions}</div>}><CreativePromptPreview result={creativePreview} /><ExperimentPromptEditor template={selectedTemplate} templateValues={templateValues} draft={draft} placeholder="描述你想生成的视频…" minRows={5} disabled={disabled} onDraftChange={setDraft} onTemplateValuesChange={setTemplateValues} onUseFreeInput={(prompt) => { setTemplateId(undefined); setTemplateValues({}); setDraft(prompt) }} /></ExperimentComposer>
  ...
  return <>{subjectEditor}{render({ history: historyNode, composer, extra, overlays, disabled: submitting })}</>
}
